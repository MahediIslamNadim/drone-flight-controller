...
_ws_discover_endpoints() {
    ...
    head -30 "$ALIVE_FILE" 2>/dev/null | while IFS= read -r BASEURL; do
        ...
        for PATH in "${WS_PATHS[@]}"; do
            local WS_TARGET="${SCHEME}://${HOST}${PATH}"
            local HTTP_TARGET="${BASEURL}${PATH}"
...
_ws_test_with_wscat() {
    ...
    command -v wscat &>/dev/null || return

    log "[WS] Testing $WS_URL with wscat..."

    # Test basic connection + injection payloads
    local -a WS_PAYLOADS=(
        '{"type":"test","msg":"hello"}'
    ...
    for PAYLOAD in "${WS_PAYLOADS[@]}"; do
        timeout 5 wscat -c "$WS_URL" \
            --no-color \
            -x "$PAYLOAD" \
            2>/dev/null >> "$WS_DIR/ws_responses_${SAFE_NAME}.txt" || true
    done
}
...
_ws_injection_test() {
    ...
    command -v wscat &>/dev/null || {
        info "[WS] wscat not found — using curl upgrade method for injection"
    }
    ...
    while IFS= read -r WS_URL; do
        _ws_test_with_wscat "$WS_URL" "$WS_DIR"
