...
generate_html_report() {
    ...
    CRITICAL=$(grep -ic "\[critical\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null || echo 0)
    HIGH=$(grep -ic "\[high\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null || echo 0)
    MEDIUM=$(grep -ic "\[medium\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null || echo 0)
    LOW=$(grep -ic "\[low\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null || echo 0)
    INFO=$(grep -ic "\[info\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null || echo 0)

    local TAKEOVER BYPASS CORS_ISSUES METHOD_BYPASS
    ...
    CLOUD_S3=$(grep -c "200-OPEN" "$OUTDIR/cloud/s3_found.txt" 2>/dev/null || echo 0)
    CLOUD_GCS=$(grep -c "200-OPEN" "$OUTDIR/cloud/gcs_found.txt" 2>/dev/null || echo 0)
    CLOUD_AZ=$(grep -c "200-OPEN" "$OUTDIR/cloud/azure_found.txt" 2>/dev/null || echo 0)
    CLOUD_FB=$(grep -c "200-OPEN" "$OUTDIR/cloud/firebase_found.txt" 2>/dev/null || echo 0)
    CLOUD_DO=$(grep -c "200-OPEN" "$OUTDIR/cloud/do_spaces_found.txt" 2>/dev/null || echo 0)
    CLOUD_R2=$(grep -c "200-OPEN" "$OUTDIR/cloud/r2_found.txt" 2>/dev/null || echo 0)
...
<div class="stats-grid">
  ...
  <div class="stat-card neutral"><div class="stat-label">Takeovers</div><div class="stat-value $([ "$TAKEOVER" -gt 0 ] && echo "high" || echo "neutral")">$TAKEOVER</div><div class="stat-sub">Candidates</div></div>
...
<div class="section">
  ...
  <div class="section-body open">
    <div class="findings-list" id="critical-list">
$(grep -i "\[critical\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null | head -50 | while IFS= read -r LINE; do
    CLEAN=$(printf '%s' "$LINE" | sed 's/&/\&amp;/g;s/"/\&quot;/g;s/</\&lt;/g;s/>/\&gt;/g')
    echo "<div class='finding-item' data-sev='critical'><span class='sev-badge critical'>CRIT</span><span class='finding-text'>$CLEAN</span></div>"
done || echo "<div style='color:var(--muted);font-size:12px'>✓ No critical findings</div>")
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <div class="section-body">
    <div class="findings-list" id="high-list">
$(grep -i "\[high\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null | head -50 | while IFS= read -r LINE; do
    CLEAN=$(printf '%s' "$LINE" | sed 's/&/\&amp;/g;s/"/\&quot;/g;s/</\&lt;/g;s/>/\&gt;/g')
    echo "<div class='finding-item' data-sev='high'><span class='sev-badge high'>HIGH</span><span class='finding-text'>$CLEAN</span></div>"
done || echo "<div style='color:var(--muted);font-size:12px'>✓ No high findings</div>")
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <div class="section-body">
    <div class="findings-list" id="medium-list">
$(grep -i "\[medium\]" "$OUTDIR/vulns/all_findings.txt" 2>/dev/null | head -30 | while IFS= read -r LINE; do
    CLEAN=$(printf '%s' "$LINE" | sed 's/&/\&amp;/g;s/"/\&quot;/g;s/</\&lt;/g;s/>/\&gt;/g')
    echo "<div class='finding-item' data-sev='medium'><span class='sev-badge medium'>MED</span><span class='finding-text'>$CLEAN</span></div>"
done || echo "<div style='color:var(--muted);font-size:12px'>✓ No medium findings</div>")
    </div>
  </div>
</div>
...
<div class="section">
  ...
  <div class="section-body">
    <table>
      ...
      <tr><td>AWS S3</td><td style="color:$([ "$CLOUD_S3" -gt 0 ] && echo '#ea580c' || echo 'var(--muted)')">$CLOUD_S3</td><td>$(cat "$OUTDIR/cloud/s3_found.txt" 2>/dev/null | grep "200-OPEN" | head -5 | tr '\n' ' ' || echo "—")</td></tr>
      <tr><td>Google GCS</td><td style="color:$([ "$CLOUD_GCS" -gt 0 ] && echo '#ea580c' || echo 'var(--muted)')">$CLOUD_GCS</td><td>$(cat "$OUTDIR/cloud/gcs_found.txt" 2>/dev/null | grep "200-OPEN" | head -5 | tr '\n' ' ' || echo "—")</td></tr>
      <tr><td>Azure Blob</td><td style="color:$([ "$CLOUD_AZ" -gt 0 ] && echo '#ea580c' || echo 'var(--muted)')">$CLOUD_AZ</td><td>$(cat "$OUTDIR/cloud/azure_found.txt" 2>/dev/null | grep "200-OPEN" | head -5 | tr '\n' ' ' || echo "—")</td></tr>
      <tr><td>Firebase</td><td style="color:$([ "$CLOUD_FB" -gt 0 ] && echo '#dc2626' || echo 'var(--muted)')">$CLOUD_FB</td><td>$(cat "$OUTDIR/cloud/firebase_found.txt" 2>/dev/null | grep "200-OPEN" | head -5 | tr '\n' ' ' || echo "—")</td></tr>
      <tr><td>DO Spaces</td><td style="color:$([ "$CLOUD_DO" -gt 0 ] && echo '#ea580c' || echo 'var(--muted)')">$CLOUD_DO</td><td>$(cat "$OUTDIR/cloud/do_spaces_found.txt" 2>/dev/null | grep "200-OPEN" | head -5 | tr '\n' ' ' || echo "—")</td></tr>
      <tr><td>Cloudflare R2</td><td style="color:$([ "$CLOUD_R2" -gt 0 ] && echo '#ea580c' || echo 'var(--muted)')">$CLOUD_R2</td><td>$(cat "$OUTDIR/cloud/r2_found.txt" 2>/dev/null | grep "200-OPEN" | head -5 | tr '\n' ' ' || echo "—")</td></tr>
    </table>
  </div>
</div>
