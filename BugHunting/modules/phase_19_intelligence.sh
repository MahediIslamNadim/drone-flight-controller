...
_fofa_lookup() {
    ...
    # Detect API error (e.g. [-700] invalid account, [-4] rate limit)
    if echo "$FOFA_RESP" | jq -e '.error' &>/dev/null 2>/dev/null; then
        local FOFA_ERR
        FOFA_ERR=$(echo "$FOFA_RESP" | jq -r '.errmsg // .error // "unknown error"' 2>/dev/null)
        warning "[FOFA] API error: $FOFA_ERR — check FOFA_EMAIL and FOFA_KEY in .env"
        return
    fi
...
phase_19_intelligence() {
    ...
    : > "$INTEL_DIR/vt_subdomains.txt"
    : > "$INTEL_DIR/st_associated.txt"
