...
EOF
    ...
    if command -v theHarvester &>/dev/null; then
        # Always-free sources (no API key required)
        local TH_SOURCES="anubis,crtsh,dnsdumpster,hackertarget,rapiddns,urlscan,waybackarchive,baidu,bing"
        # Add sources conditionally based on available API keys
        [ -n "$SHODAN_API_KEY" ]         && TH_SOURCES="${TH_SOURCES},shodan"
        [ -n "$HUNTER_API_KEY" ]         && TH_SOURCES="${TH_SOURCES},hunter"
        [ -n "$SECURITYTRAILS_API_KEY" ] && TH_SOURCES="${TH_SOURCES},securitytrails"
        [ -n "$VIRUSTOTAL_API_KEY" ]     && TH_SOURCES="${TH_SOURCES},virustotal"
        [ -n "$CENSYS_API_ID" ]          && TH_SOURCES="${TH_SOURCES},censys"
        [ -n "$FULLHUNT_API_KEY" ]       && TH_SOURCES="${TH_SOURCES},fullhunt"
        info "[8.1] theHarvester sources: $TH_SOURCES"
        theHarvester -d "$DOMAIN" -b "$TH_SOURCES" \
