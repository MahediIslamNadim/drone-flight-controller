...
_test_redis() {
    ...
    local RESP
    RESP=$(printf '*1\r\n$4\r\nINFO\r\n' | timeout 5 nc -w 1 "$IP" "$PORT" 2>/dev/null | head -20)
...
_test_memcached() {
    ...
    local RESP
    RESP=$(printf 'stats\r\n' | timeout 4 nc -w 1 "$IP" "$PORT" 2>/dev/null | head -5)
...
_test_admin_panels() {
    ...
    local -a ADMIN_PORTS=(8080 8443 9000 9090 3000 8888)

    head -20 "$ALIVE_FILE" 2>/dev/null | while IFS= read -r BASEURL; do
        BASEURL="${BASEURL%/}"
        local HOST
        HOST=$(echo "$BASEURL" | awk -F/ '{print $3}' | cut -d: -f1)
        local SCHEME="${BASEURL%%://*}"
        ...
        # Probe alternate management ports as standalone URLs
        for PORT in "${ADMIN_PORTS[@]}"; do
            local PORT_SCHEME="http"
            [[ "$PORT" == "8443" ]] && PORT_SCHEME="https"
            local PORT_URL="${PORT_SCHEME}://${HOST}:${PORT}/"
            local RESP_CODE
            RESP_CODE=$(curl_safe -sk -o /dev/null -w "%{http_code}" "$PORT_URL" --max-time 5 2>/dev/null || echo "000")
            if [[ "$RESP_CODE" =~ ^2 ]]; then
                echo "${PORT_URL} [HTTP $RESP_CODE]" >> "$INFRA_DIR/admin_panels.txt"
                info "[Admin] Found port service: $PORT_URL"
            fi
        done &
    done
    wait
