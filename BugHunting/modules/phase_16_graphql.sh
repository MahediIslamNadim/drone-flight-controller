...
phase_16_graphql() {
    ...
    log "[GQL] Fingerprinting GraphQL engine with graphw00f..."
    if command -v graphw00f &>/dev/null; then
        while IFS= read -r ENDPOINT; do
            graphw00f -d -t "$ENDPOINT" \
                2>>"$OUTDIR/logs/errors.log" | \
                grep -iE "detected|engine|version" >> "$GQL_DIR/engine_fingerprint.txt" || true
        done < "$GQL_DIR/endpoints.txt"
        [ -s "$GQL_DIR/engine_fingerprint.txt" ] && {
            success "graphw00f: $(wc -l < "$GQL_DIR/engine_fingerprint.txt") fingerprint hits"
            cat "$GQL_DIR/engine_fingerprint.txt"
        }
    else
        info "[GQL] graphw00f not installed — skipping engine fingerprint (pip install graphw00f)"
    fi
    ...
    local INTRO_CNT FIELD_CNT BATCH_CNT MUT_CNT FP_CNT
    ...
    FP_CNT=$(wc -l < "$GQL_DIR/engine_fingerprint.txt" 2>/dev/null || echo 0)

    success "PHASE 16 DONE — Endpoints: $GQL_COUNT | Engine FP: $FP_CNT | Introspect bypass: $INTRO_CNT | Fields: $FIELD_CNT | Mutations: $MUT_CNT | Time: $(( $(date +%s) - START_TIME ))s"
